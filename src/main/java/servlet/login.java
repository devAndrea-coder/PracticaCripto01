...
public class login extends HttpServlet {
    ...
    public void init() throws ServletException {
        super.init();
        try {
            emf = Persistence.createEntityManagerFactory("com.mycompany_CriptoPractica_war_1.0-SNAPSHOTPU");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Estudiante estudiante = buscarEstudiantePorNdniWeb(dni);
            ...
            if (estudiante != null) {
                ...
                if (passwordMatch) {
                    ...
                    request.getSession().setAttribute("usuario", estudiante.getLogiEstd());
                    
                    json.put("status", "ok");
                    json.put("redirect", "tabla.html");
                    json.put("usuario", estudiante.getLogiEstd());
                    json.put("dni", dni);
                    json.put("loginMethod", "normal");
                    
                    System.out.println("INFO - Login exitoso para DNI: " + dni + " Usuario: " + estudiante.getLogiEstd());
                } else {
     ...
     */
    private Estudiante buscarEstudiantePorNdniWeb(String ndniWeb) {
        try {
            ...
            if (estudiante == null) {
                try {
                    List<Estudiante>  todos = loginService.findEstudianteEntities();
                    for (Estudiante est : todos) {
                        if (est.getNdniEstdWeb() != null && est.getNdniEstdWeb().equals(ndniWeb)) {
                            estudiante = est;
                            break;
                        }
                    }
            ...
            if (estudiante != null) {
                ...
                System.out.println("  - Código: " + estudiante.getCodiEstdWeb());
                System.out.println("  - DNI Web: " + estudiante.getNdniEstdWeb());
                System.out.println("  - Login: " + estudiante.getLogiEstd());
                System.out.println("  - Nombre: " + estudiante.getAppaEstdWeb() + ", " + estudiante.getApmaEstdWeb());
                System.out.println("  - Tiene password: " + (estudiante.getPassEstd() != null ? "Sí" : "No"));
            } else {
